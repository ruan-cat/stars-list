...
- 先区分响应式 viewport：只有同尺寸、同主题、同加载状态才做 pixel diff；原始/归一化 diff 作为诊断，不把跨尺寸百分比直接当成产品回归。真正的硬门禁改为结构性结论（双滚动、窄屏堆叠、marker/缩进、主题变量、sticky 动作栏）。
...
## 8. 用户纠偏补充：响应式页面不采用统一像素硬门禁

用户进一步指出页面是响应式布局，不能把 `1600×1000` 基线与 `1280×900`/`720×900` fresh 截图的差异当成产品失败。复核后确认：§15 的 `4.60%` 仅是两张同尺寸图的诊断事实，不能外推到不同 viewport，也不能否定已经通过的 TODO 功能与响应式结构。后续规则改为：分别验收 `1280×900` 桌面和 `720×900` 窄视口；只有同尺寸、同主题、同加载状态的基线才执行 pixel diff；原始/归一化 diff 只作诊断，不设跨 viewport 的统一百分比硬阈值；结构性事实（双滚动、窄屏堆叠、marker/缩进、主题变量、sticky 动作栏）才是视觉硬门禁。该补充 supersede 本报告早先“设定统一像素阈值”的表述。
