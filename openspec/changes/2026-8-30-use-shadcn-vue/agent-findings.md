...
对照事故报告第 5 节复核后，补齐两处原先未完全落实的建议：任务/spec/design/manifest/技能现在都明确可选 A2 reload 探针（仅一次，失败只阻塞 C 的 reload 子项），并曾定义 mask 后归一化 diff `≤1%` 参考阈值与 `>1%` 独立解释规则。经用户针对响应式页面的进一步纠偏，该统一百分比阈值已由 F45 撤销，改为同 viewport 诊断 diff + 结构性结论；产品/故障分层、规范化资源清单 + SHA、独立只读 verifier、二次同类控制面故障止损和不跨 session 规则仍存在且口径一致。未修改任务勾选。
