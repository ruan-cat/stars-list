...
- [ ] 4.2 亮/暗主题视觉回归：固定 URL、viewport、滚动位置、字体加载和 artifact 版本；对动态时间、光标、网络状态等区域使用明确 mask 或单独断言，同时记录原始 diff、归一化 diff 和结构性视觉结论。原始 diff 只作诊断；对 mask 后的归一化 diff 采用 `≤1%` 作为“可接受但仍需结构性检查”的参考阈值，`>1%` 必须由独立复核解释，不自动判定产品失败；出现无法解释的结构漂移、marker、页面级滚动、主题变量失效时才判定失败。基线缺少元数据时只能标记 `参考`，不能伪造 before/after 通过
