...
- [ ] 4.1 统一验收协调：dev/preview/production 各自先执行一次不超过 5 分钟的基础能力探针（`skills get core`、`doctor --offline --quick`、session 注册、headed 前台 visibility、一次截图、network requests、session close）；基础探针失败立即标记 `blocked` 并停止该环境。若 C 阶段确实需要 reload，再追加一次可选控制面探针（artifact `route --abort` → 单次 reload → 原 session 恢复）；该可选探针失败只将 C 的 reload 子项标记 `blocked`，不抹除已通过的 B 产品证据，也不得再次重试。探针通过后，读取并登记 `evidence/01~08-*.png` 对应基线（绝对路径、尺寸、SHA-256、可见事实），再按 spec Scenario 记录唯一 session、命令、截图、`a11y --json` 的 violations/incomplete、DOM/网络/console 与证据状态；不得跨环境或跨 session 拼接。
- [ ] 4.2 响应式视觉回归：分别在桌面 `1280×900` 与窄视口 `720×900` 验收，固定 URL、滚动位置、字体加载和 artifact 版本；对动态时间、光标、网络状态等区域使用明确 mask 或单独断言。只有同 viewport、同主题、同加载状态的基线才执行 pixel diff；原始/归一化 diff 仅作诊断，不设跨 viewport 的统一百分比硬阈值。硬门禁改为结构性视觉结论：不得出现 marker/异常缩进、页面级双滚动、窄视口未堆叠、主题变量失效、sticky 动作栏裁切；基线缺少元数据或尺寸不一致时只能标记 `参考`，不能把 `1600×1000` 与 `1280×900` 的 diff 当成失败或通过
...
- reviewer 只读核对任务源、manifest、截图路径、PNG 尺寸、SHA-256、日志和状态映射；在已有 session 的收据中复核 `a11y --json --tags wcag2a,wcag2aa` 的 violations/incomplete、语义树和键盘断言；不要求为了“独立”再开第二个浏览器。
